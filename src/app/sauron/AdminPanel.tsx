...
export default function AdminPanel({ poll }: { poll: Poll | null }) {
  ...
  // NOTE: both actions below are scoped to week_type = "current" only.
  // The "next" week (auto-generated by the Friday rollover) is left untouched
  // here — manage it by waiting for the weekly rollover, or extend this panel
  // if you need manual control over next week too.

  async function handleResetVotes() {
    if (!poll) return;
    if (!confirm("This will clear all votes for the current week but keep the dates. Are you sure?")) return;
    ...
    try {
      ...
      showMessage("Current week's votes cleared. Dates kept intact.", "success");
      router.refresh();
    } catch (err) {
  ...
  async function handleResetPoll() {
    if (!poll) return;
    if (!confirm(`This will clear the current week's votes & dates, then create a new current week starting ${formatMonday(selectedMonday)}. The "next week" poll is not affected. Are you sure?`)) return;

    setIsResettingPoll(true);
    setMessage("");

    try {
      // Fetch existing current-week dates
      ...
      showMessage(`Current week reset to week of ${formatMonday(selectedMonday)}.`, "success");
      router.refresh();
    } catch (err) {
      showMessage(err instanceof Error ? err.message : "Something went wrong.", "error");
    } finally {
  ...
  return (
    <div className="border rounded-xl p-6 space-y-8">
      ...
      <div className="space-y-2">
        <h2 className="text-xl font-semibold">Reset Votes (current week)</h2>
        <p className="text-sm text-gray-400">
          Clears votes for the current week but keeps its dates intact. Next week is untouched.
        </p>
        <button
      ...
      <div className="space-y-3">
        <h2 className="text-xl font-semibold">Reset Poll (current week)</h2>
        <p className="text-sm text-gray-400">
          Clears votes & dates for the current week, then creates Mon–Fri for the selected week. Next week is untouched.
        </p>
